...
int main(int argc, char *argv[])
{
  while(1)
  {
    ...
    int totalb;
    int totalc;
    totala = stack[0][0]+stack[1][0]+stack[2][0]+stack[3][0]+stack[4][0];
    totalb = stack[0][1]+stack[1][1]+stack[2][1]+stack[3][1]+stack[4][1];
    totalc = stack[0][2]+stack[1][2]+stack[2][2]+stack[3][2]+stack[4][2];
    int loop0;
	...
	if(loop1 == 0 && loop2 == 0)
	{
	  if(stack[4][input3] == 0) // Check that the stack from which you want to take is not empty
	  {
	    loop3 = 1;
	  ...
	  else
	  {
		...
		y = y-1;
	      ...
	      if(y == 4 && stack[y][input4] == 0) // Check that the second disc of which you want to put the first disc is bigger than the first
	      {
		loop5 = 0;
	      }
	      else
